...
use crate::hooks::a2c::{A2CMinibatchStats, A2CRolloutStats};
...
impl A2CRolloutReporter {
    ...
    /// Publishes statistics for the rollout identified by the coordinator.
    ///
    /// # Arguments
    ///
    /// * `rollout_idx` - One-based index of the rollout being learned.
    /// * `total_rollouts` - Planned rollout count, if known.
...
///
/// Applies learning-rate scheduling before learning using shared collection progress.
///
...
pub(crate) struct A2CLearningHook<T = ()> {
    ...
    for A2CLearningHook<BurnPolicyValueLearner<B, D>>
{
    ...
    fn batch_hook(
        ...
        if let Some(A2CRolloutReporter { report, .. }) = &mut self.reporter {
            report.minibatch_stats.push(A2CMinibatchStats {
