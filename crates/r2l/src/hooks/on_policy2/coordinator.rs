...
impl Coordinator {
    ...
    pub(crate) fn update_rollout_progress<A: Agent, S: Sampler>(
        ...
        self.training_state.steps_taken += rollouts
            .as_ref()
            .iter()
            .map(|trajectory| trajectory.actions.len())
            .sum::<usize>();
        self.training_state.collected_rollouts += 1;
    }

    pub(crate) fn finish_rollout(&mut self) {
    ...
    pub(crate) fn progress_remaining(&self) -> f64 {
        let remaining = match self.training_limit {
            TrainingLimit::RolloutBound { total_rollouts } => {
                1.0 - self.training_state.collected_rollouts as f64 / total_rollouts as f64
            }
...
pub(crate) struct SamplerHookImpl<E: Env> {
