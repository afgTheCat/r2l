...
use r2l_core::agents::Agent5;
use r2l_core::sampler4::buffer::{BufferS, TrajectoryContainer};
...
impl<M: ModuleWithValueFunction, H: PPOHooksTrait5<M>> Agent5 for CandlePPO5<M, H> {
    ...
    fn learn4<T: r2l_core::tensor::R2lTensor, C: TrajectoryContainer<Tensor = T>>(
        &self,
        buffers: &[BufferS<C>],
    ) -> Result<()> {
        todo!()
