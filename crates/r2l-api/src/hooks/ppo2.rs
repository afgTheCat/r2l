...
use r2l_agents::on_policy_algorithms::{
    Advantages, Returns,
    ppo2::{PPOBatchData, PPOHook, PPOParams},
};
...
/// Per-batch training statistics emitted by the default PPO2 hook.
///
/// Each value corresponds to a single optimization batch processed within one
/// PPO epoch.
#[derive(Debug, Clone)]
pub struct PPO2BatchStats {
...
/// Aggregated statistics emitted by the default PPO2 hook after a learning
/// pass.
///
/// A report contains all collected [`PPO2BatchStats`] for the rollout together
/// with rollout-level summaries such as average reward and learning rate.
#[derive(Default, Debug, Clone)]
pub struct PPO2Stats {
    ...
    pub batch_stats: Vec<PPO2BatchStats>,
...
impl PPO2Stats {
    ...
    pub fn collect_batch_data(&mut self, batch_stats: PPO2BatchStats) {
        self.batch_stats.push(batch_stats);
    }
}

impl std::fmt::Display for PPO2Stats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(f, "PPO2 stats (rollout {})", self.rollout_idx)?;
...
pub(crate) struct DefaultPPO2HookReporter {
    report: PPO2Stats,
...
impl DefaultPPO2HookReporter {
    pub fn new(tx: Option<Sender<PPO2Stats>>, log_progress: bool, n_envs: usize) -> Option<Self> {
        if tx.is_some() || log_progress {
            Some(Self {
                report: PPO2Stats::default(),
                tx,
                log_progress,
...
/// Default training hook used by [`PPO2AgentBuilder`](crate::PPO2AgentBuilder).
///
/// This hook applies the crate's standard PPO2 training behavior: advantage
...
pub struct DefaultPPO2Hook<T = ()> {
    ...
    pub(crate) reporter: Option<DefaultPPO2HookReporter>,
    ...
    for DefaultPPO2Hook<BurnPolicyValueModule<B, D>>
{
    ...
    fn batch_hook(
        ...
        if let Some(DefaultPPO2HookReporter { report, .. }) = &mut self.reporter {
            let ratio: Vec<f32> = data.ratio.to_data().to_vec().unwrap();
            let clip_fraction = ratio
                ...
                .count() as f32
                / ratio.len() as f32;
            report.collect_batch_data(PPO2BatchStats {
                clip_fraction,
