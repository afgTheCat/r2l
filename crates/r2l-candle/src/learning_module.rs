...
impl<P: CandlePolicy> PolicyValueLearner<P> {
    ...
    pub fn build_joint(
        ...
        let policy_vb = crate::random::seeded_var_builder(&policy_varmap, DType::F32, &device);
    ...
    pub fn build_split(
        ...
        let critic_vb = crate::random::seeded_var_builder(&critic_varmap, DType::F32, &device);
        let value_layers = &[value_hidden_layers, &[1]].concat();
        let value_function = SequentialValueFunction::new(
            observation_size,
            value_layers,
